...
-- 1. ORDER BY: Mengurutkan item berdasarkan harga termahal
SELECT * FROM items 
ORDER BY harga DESC;

-- 2. DISTINCT: Menampilkan id_items yang unik dari tabel transaksi_detail
SELECT DISTINCT id_items FROM transaksi_detail;

-- 3. OPERATOR LOGIKA (AND, OR, NOT): Mencari item dengan harga di atas 40rb DAN stok di atas 10
SELECT * FROM items 
WHERE harga > 40000 AND stock > 10;

-- 4. LIKE: Mencari item yang mengandung kata 'Liquid'
SELECT * FROM items 
WHERE nama_item LIKE '%Liquid%';

-- 5. BETWEEN: Mencari item dengan stok antara 10 sampai 40
SELECT * FROM items 
WHERE stock BETWEEN 10 AND 40;

-- 6. CALCULATION & ALIAS: Menghitung estimasi pajak 10% per item
SELECT 
    nama_item, 
    harga AS harga_asli, 
    (0.1 * harga) AS estimasi_ppn, 
    (harga + (0.1 * harga)) AS harga_total_pajak
FROM items;

-- 7. FUNGSI AGREGAT LAINNYA (MIN, MAX, AVG): Statistik harga barang
SELECT 
...
-- E. RIGHT JOIN: Menampilkan semua data Operator dan mencocokkan dengan data Karyawan
SELECT 
    k.nama_lengkap, 
    o.username 
FROM karyawan k 
RIGHT JOIN operator o ON k.id_karyawan = o.id_karyawan;

-- F. FULL JOIN (Simulasi menggunakan UNION): Menampilkan semua data Karyawan dan Operator
SELECT k.nama_lengkap, o.username 
FROM karyawan k 
LEFT JOIN operator o ON k.id_karyawan = o.id_karyawan
UNION
SELECT k.nama_lengkap, o.username 
